...
_SUBSCRIPT_MAP = str.maketrans({"0":"₀","1":"₁","2":"₂","3":"₃","4":"₄","5":"₅","6":"₆","7":"₇","8":"₈","9":"₉","+":"₊","-":"₋","=":"₌","(":"₍",")":"₎","i":"ᵢ","j":"ⱼ","n":"ₙ"})
_SUPERSCRIPT_MAP = str.maketrans({"0":"⁰","1":"¹","2":"²","3":"³","4":"⁴","5":"⁵","6":"⁶","7":"⁷","8":"⁸","9":"⁹","+":"⁺","-":"⁻","=":"⁼","(":"⁽",")":"⁾","i":"ⁱ","n":"ⁿ"})
...
def _normalize_symbolic_text(text: str) -> str:
    ...
    normalized = re.sub(r"([A-Za-zα-ωΑ-Ω])_\{?([0-9ijn+\-=()]+)\}?", lambda m: f"{m.group(1)}{m.group(2).translate(_SUBSCRIPT_MAP)}", normalized)
    normalized = re.sub(r"([A-Za-zα-ωΑ-Ω])\^\{?([0-9in+\-=()]+)\}?", lambda m: f"{m.group(1)}{m.group(2).translate(_SUPERSCRIPT_MAP)}", normalized)
...
def _is_rich_formula_text(text: str) -> bool:
    ...
    if not text:
        return False
    rich_markers = ("=", "/", "Σ", "∑", "⟺", "⇔", "∀", "∈", "lim", "arccos", "sqrt", "√", "matrix", "[", "]", "{", "}")
    return any(marker in text for marker in rich_markers) or (" " in text and len(text) > 12) or len(text) > 18


def _choose_label_font_family(text: str, *, formula: bool, rich_formula: bool) -> str:
    if not formula or rich_formula:
        return MPL_BODY_FONT
    simple_token = len(text) <= 6 and " " not in text and not any(ch in text for ch in ("=", "/", "[", "]", "{", "}"))
    return MPL_FORMULA_FONT if simple_token else MPL_BODY_FONT


def _label_text(ax, x: float, y: float, text: str, p, *, size: float = 8.0, formula: bool = False, ha: str = "center", va: str = "center", color=None, allow_rich_formula: bool = False, fallback_text: str | None = None) -> None:
    ...
    ax.text(x, y, render_text, ha=ha, va=va, fontsize=size * (0.94 if rich_formula else 1.0), color=p["fg"] if color is None else color, fontfamily=_choose_label_font_family(render_text, formula=formula, rich_formula=rich_formula), fontstyle="normal", math_fontfamily="dejavusans", linespacing=1.0, clip_on=False)


def _rounded_box(ax, x: float, y: float, w: float, h: float, p, *, lw: float = 1.3, fill=None) -> None:
    ax.add_patch(mpatches.FancyBboxPatch((x, y), w, h, boxstyle="round,pad=0.02,rounding_size=0.18", linewidth=lw, edgecolor=p["fg"], facecolor=p["panel"] if fill is None else fill))
...
def _right_angle_marker(ax, x: float, y: float, size: float, p) -> None:
    ax.add_patch(mpatches.Rectangle((x, y), size, size, fill=False, edgecolor=p["ghost"], linewidth=0.9))
...
def _movie_icon(ax, x: float, y: float, w: float, h: float, p) -> None:
    ...
    tri_w = w * 0.22
    tri_h = h * 0.24
    ax.add_patch(mpatches.Polygon([(cx - tri_w * 0.40, cy - tri_h / 2), (cx - tri_w * 0.40, cy + tri_h / 2), (cx + tri_w * 0.60, cy)], closed=True, edgecolor=p["accent"], facecolor=p["accent"], linewidth=1.0))
...
def _vector_strip(ax, x: float, y: float, w: float, h: float, p, values: list[str], *, labels: list[str] | None = None) -> None:
    gap = 0.12
    cell_w = (w - gap * (len(values) - 1)) / len(values)
    for i, v in enumerate(values):
        cell_x = x + i * (cell_w + gap)
        face = p["bg_soft"] if v not in {"1", "x"} else p["accent"]
        ax.add_patch(mpatches.Rectangle((cell_x, y), cell_w, h, linewidth=1.2, edgecolor=p["fg"], facecolor=face))
        txt_color = (1, 1, 1) if v in {"1", "x"} else p["fg"]
        _label_text(ax, cell_x + cell_w / 2, y + h / 2, v, p, size=10, formula=True, color=txt_color)
        if labels and i < len(labels):
            _label_text(ax, cell_x + cell_w / 2, y - 0.16, labels[i], p, size=6.8, ha="center", va="top", color=p["soft"])
...
def _vector_arrow(ax, start: tuple[float, float], end: tuple[float, float], p, *, color=None, lw: float = 2.2, label: str | None = None, label_dx: float = 0.12, label_dy: float = 0.12) -> None:
...
def _draw_checkmark(ax, cx: float, cy: float, size: float, color, *, lw: float = 2.2) -> None:
    ax.plot([cx - size * 0.40, cx - size * 0.10], [cy - size * 0.02, cy - size * 0.28], color=color, lw=lw, solid_capstyle="round")
    ax.plot([cx - size * 0.10, cx + size * 0.42], [cy - size * 0.28, cy + size * 0.28], color=color, lw=lw, solid_capstyle="round")
...
__all__ = [
    "ACCENT", "BODY_FONT", "FORMULA_FONT", "GENERATED_DIR", "NAVY", "OFFWHITE", "SLATE", "add_image", "add_textbox",
    "palette_for", "_canvas", "_save", "_label_text", "_rounded_box", "_soft_panel", "_mini_panel", "_right_angle_marker",
    "_clean_origin_axes", "_movie_icon", "_digit_card_shape", "_vector_strip", "_axes_2d", "_axes_3d_fake", "_vector_arrow",
    "_draw_checkmark", "_tip_projection", "_clear_fill",
]
