...
def _make_norm_worked_geometry(path: Path, variant: str) -> Path:
    """Wider and taller inner panel so the left analytic diagram is not squeezed."""
    p = palette_for(variant)
...
def _make_dot_product_worked_geometry(path: Path, variant: str) -> Path:
    p = palette_for(variant)
    fig, ax = _canvas(path, figsize=(5.15, 3.05))
    _mini_panel(ax, 0.78, 0.92, 4.28, 4.15, p)
    _mini_panel(ax, 5.30, 0.92, 3.92, 4.15, p)
    _vector_strip(ax, 1.12, 3.18, 3.50, 0.76, p, ["3", "2", "2"], labels=["x₁", "x₂", "x₃"])
    _vector_strip(ax, 1.12, 1.76, 3.50, 0.76, p, ["1", "1", "1"], labels=["y₁", "y₂", "y₃"])
    for x in [1.70, 2.86, 4.02]:
        ax.plot([x, 6.05], [2.50, 2.98], color=p["ghost"], lw=0.9)
    _label_text(ax, 7.28, 3.90, "matching entries", p, size=7.95, color=p["soft"])
    _label_text(ax, 7.28, 3.00, "multiply", p, size=8.7, color=p["fg"])
    _label_text(ax, 7.28, 2.20, "then add", p, size=8.0, color=p["soft"])
    return _save(fig, path)
...
def _make_angle_homework_geometry(path: Path, variant: str) -> Path:
    """Give the geometry much more vertical breathing room for title/analytic use."""
    p = palette_for(variant)
    fig, ax = _canvas(path, figsize=(6.0, 3.55))
    _mini_panel(ax, 0.55, 0.70, 8.95, 4.65, p)
    origin = (1.25, 1.18)
    x_tip = (5.85, 4.12)
    y_tip = (7.65, 1.18)
    _vector_arrow(ax, origin, x_tip, p, color=p["accent"], lw=2.45, label="x", label_dx=0.16, label_dy=0.08)
    _vector_arrow(ax, origin, y_tip, p, color=p["soft"], lw=2.2, label="y", label_dx=0.14, label_dy=0.02)
    ax.plot([x_tip[0], x_tip[0]], [x_tip[1], origin[1]], color=p["ghost"], lw=0.95, linestyle="--")
    _right_angle_marker(ax, 5.52, 1.18, 0.30, p)
    ax.add_patch(mpatches.Arc(origin, 1.82, 1.24, angle=0, theta1=8, theta2=33, color=p["ghost"], lw=1.05))
    _label_text(ax, 2.78, 1.78, "α", p, size=8.9, formula=True)
    return _save(fig, path)
