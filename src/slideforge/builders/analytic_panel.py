...
def _estimate(text: str, width: float, min_font: int, max_font: int, max_lines: int | None, *, extra: float = 0.05) -> float:
    if not _clean(text) or width <= 0:
        return 0.0
    fitted = fit_text(text, width * 0.97, 10.0, min_font_size=min_font, max_font_size=max_font, max_lines=max_lines)
...
def _style(spec: Mapping[str, Any], theme_obj) -> dict[str, Any]:
    raw = dict(spec.get("worked_example_style", {}) or spec.get("analytic_panel_style", {}) or {})
    fill = theme_obj.box_fill_color or theme_obj.panel_fill_color or OFFWHITE
    surface_fill = resolve_color(raw.get("surface_fill_color"), fill)
...
def _content(spec: Mapping[str, Any]) -> dict[str, Any]:
    ...
    steps = _normalize_steps(spec.get("steps"), explanation if len(_lines(spec.get("steps"))) >= 3 else "")
    explanation_box_text = "" if explanation and len(_lines(spec.get("steps"))) >= 3 else explanation
    return {
...
def _text_card(slide, box: Box, label: str, text: str, style: Mapping[str, Any], color, min_font: int, max_font: int, max_lines: int | None, *, bold: bool = False) -> None:
    ...
    inner = Box(box.x + 0.16, box.y + 0.34, max(0.0, box.w - 0.32), max(0.0, box.h - 0.48))
...
def _visual_card(slide, box: Box, content: Mapping[str, Any], style: Mapping[str, Any]) -> None:
    ...
    if content["visual_caption"]:
        caption_h = max(0.18, min(0.36, _estimate(content["visual_caption"], max(0.1, box.w - 0.36), 9, 11, 2, extra=0.02)))
    img_box = Box(box.x + 0.16, box.y + 0.30, max(0.0, box.w - 0.32), max(0.0, box.h - 0.44 - caption_h))
    ...
    if caption_h > 0:
        ...
        add_textbox(slide, x=cap.x, y=cap.y, w=cap.w, h=cap.h, text=content["visual_caption"], font_name=BODY_FONT, font_size=_fit(content["visual_caption"], cap, 9, 11, 2), color=style["body_color"], align=PP_ALIGN.CENTER)


def _dense_score(content: Mapping[str, Any]) -> int:
    return len(content["steps"]) * 2 + len(content["result_lines"]) + (1 if content["takeaway"] else 0) + (1 if content["explanation"] else 0)
...
def build_analytic_panel_slide(prs: Presentation, spec: dict[str, Any], counters: dict[str, int]) -> None:
    ...
    # Safer analytic defaults: protect text first.
    mode = _clean(layout.get("worked_layout_mode") or layout.get("layout_mode") or "two_column").lower()
    density = _dense_score(content)
    layout.setdefault("worked_layout_mode", mode or "two_column")
    ...
    if layout["worked_layout_mode"] == "top_visual":
        layout.setdefault("diagram_min_share", 0.22)
        layout.setdefault("diagram_preferred_share", 0.28)
        layout.setdefault("diagram_max_share", 0.40)
        layout.setdefault("min_steps_h", 1.90 if density >= 8 else 1.65)
        layout.setdefault("result_min_h", 0.76)
        layout.setdefault("result_max_h", 1.18)
        layout.setdefault("takeaway_min_h", 0.54)
        layout.setdefault("takeaway_max_h", 0.90)
    else:
        layout.setdefault("diagram_min_share", 0.22)
        layout.setdefault("diagram_preferred_share", 0.27 if density >= 8 else 0.30)
        layout.setdefault("diagram_max_share", 0.38)
        layout.setdefault("min_steps_h", 2.10 if density >= 8 else 1.75)
        layout.setdefault("explanation_min_h", 0.34)
        layout.setdefault("explanation_max_h", 0.62)
        layout.setdefault("result_min_h", 0.68)
        layout.setdefault("result_max_h", 1.04)
        layout.setdefault("takeaway_min_h", 0.48)
        layout.setdefault("takeaway_max_h", 0.82)
    ...
    layout_result = layout_analytic_panel(
        ...
        layout_mode=layout["worked_layout_mode"],
        ...
        diagram_min_share=float(layout.get("diagram_min_share", 0.22)),
        diagram_preferred_share=float(layout.get("diagram_preferred_share", 0.28)),
        diagram_max_share=float(layout.get("diagram_max_share", 0.38)),
        min_steps_h=float(layout.get("min_steps_h", 1.80)),
        explanation_min_h=float(layout.get("explanation_min_h", 0.34)),
        explanation_max_h=float(layout.get("explanation_max_h", 0.62)),
        result_min_h=float(layout.get("result_min_h", 0.68)),
        result_max_h=float(layout.get("result_max_h", 1.02)),
        takeaway_min_h=float(layout.get("takeaway_min_h", 0.48)),
        takeaway_max_h=float(layout.get("takeaway_max_h", 0.82)),
    )

    _visual_card(slide, layout_result.diagram_box, content, style)
    ...
    if layout_result.steps_box.h > 0 and content["steps"]:
        ...
        inner = Box(layout_result.steps_box.x + 0.16, layout_result.steps_box.y + 0.34, max(0.0, layout_result.steps_box.w - 0.32), max(0.0, layout_result.steps_box.h - 0.48))
        render_compact_derivation_stack(slide, box=inner, steps=content["steps"], style=style["math"], min_body_font=10, max_body_font=12, min_formula_font=11, max_formula_font=13, final_answer="", emphasize_final_answer=False, align=PP_ALIGN.LEFT)
    if layout_result.result_box.h > 0 and content["result_lines"]:
        render_result_callout(slide, box=layout_result.result_box, result_lines=content["result_lines"], label=content["result_label"], style=style["math"], min_font=11, max_font=15, emphasize_final_answer=True, align=PP_ALIGN.LEFT, draw_card=True)
